...
io.on('connect', socket => {
    ...
    socket.on("join room", (roomid, username) => {
        ...
        if (rooms[roomid] && rooms[roomid].length > 0) {
            ...
            socket.to(roomid).emit('message', `${username} joined the room.`, 'System', moment().format(
                "h:mm a"
            ));
    ...
    socket.on('disconnect', () => {
        ...
        socket.to(socketroom[socket.id]).emit('message', `${socketname[socket.id]} left the chat.`, `System`, moment().format(
            "h:mm a"
        ));
